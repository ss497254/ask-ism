...
export const AnsChatList = ({ room, userMap }) => {
    const { setData } = useContext(UserPreviewModalContext);
    const { messages, toggleFrozen } = useAnsChatStore();
    const me = useConn().user;
    const { isMod: iAmMod, isCreator: iAmCreator } = useCurrentAnsInfo();
    const bottomRef = (useRef < null) | (HTMLDivElement > null);
    const chatListRef = (useRef < null) | (HTMLDivElement > null);
    const {
        isAnsChatScrolledToTop,
        setIsAnsChatScrolledToTop,
        message,
        setMessage,
    } = useAnsChatStore();
    ...
    const getBadgeIcon = (m) => {
        ...
        if (isCreator) {
            ...
            badge = (
                <Emote title="Mod" alt="mod" size="small" emote="ask-the-ism" />
            );
    ...
    return (
        <div
            ...
            onScroll={() => {
                ...
                setIsAnsChatScrolledToTop(!isOnBottom);
                if (isOnBottom) {
        ...
        >
            ...
            >
                {rowVirtualizer.virtualItems.map(
                    ({ index: idx, start, measureRef, size }) => {
                                            ...
                                            <div className={`inline`}>
                                                {messages[index].deleted ? (
                                                    <span className="inline text-primary-300 italic">
                                                        {"roomChat.messageDeletion.message" +
                                                            ""}
                                                        {messages[index]
                                                            .deleterId ===
                                                        messages[index].userId
                                                            ? "roomChat.messageDeletion.retracted"
                                                            : "roomChat.messageDeletion.deleted"}
                                                    </span>
                                                ) : (
                ...
                {/* {messages.length === 0 ? (
        <div className="flex">{("roomChat.welcomeMessage")}</div>
      ) : null} */}
            </div>
            <div ref={bottomRef} />
        </div>
